...
class QuickDrawDataset(data.Dataset):
    ...
    object" images
    """
    def __init__(self, list_IDs, labels, path=DIR_NAME, img_size=224):
        """
    ...
    def __getitem__(self, index, decode=True):
        ...
        """
        filename = os.path.join(self.path, self.list_IDs[index])
    ...
    def get_random_composite_drawing(self, num_components):
        ...
        def is_valid_point(boxes, point, threshold):
            x, y = point
            for box in boxes:
                if (x - box[0])**2 + (y - box[1])**2 < threshold**2:
                    return False
            return True
        ...
        scale_mean = np.sqrt(0.6 / num_components)
        scale_stdev = 0.2
        scales = np.random.normal(scale_mean, scale_stdev, num_components).clip(0, 1)

        # Threshold minimum distance between anchor points for drawings
        threshold = 1.2 / num_components
        ...
        for i in range(num_components):
            ...
            while not is_valid_point(boxes, (x1, y1), threshold):
                x1, y1 = get_anchor_point(scale, bounds)
            box = [x1, y1, (int)(x1 + scale * 256), (int)(y1 + scale * 256)]
            boxes.append(box)
